...
class ProcessingParams:
    ...
    - th_mask (0.1): Threshold for mask creation (0-1)
    - th_streak (3): Threshold for streak detection
    - win_streak (3): Window size for streak detection
    - exp_donut (9): Exponent for donut detection
    - exp_streak (3): Exponent for streak detection
    """
    ...
    def validate(self) -> None:
        ...
        try:
            ...
            if exp_donut < 0:
                raise ValueError('Exponent of donut should be a non-negative integer')
        except ValueError as e:
            if 'invalid literal for int()' in str(e):
                raise ValueError('Exponent of donut should be an integer') from e
            raise
        ...
        try:
            ...
            if exp_streak < 0:
                raise ValueError('Exponent of streak should be a non-negative integer')
        except ValueError as e:
            if 'invalid literal for int()' in str(e):
                raise ValueError('Exponent of streak should be an integer') from e
            raise
